import os
import time
# Importar las clases desde los archivos
from Paciente import Paciente
from HistoriaClinica import HistoriaClinica
from Cama import Cama
from Medicamentos import Medicamentos

# Crear una lista de usuarios y contraseñas
...
def abrir_menu():
    ...
    if(opcion_escogida=='1'):
        ...
        documento_paciente_buscar = input("Ingrese el documento del paciente que desea buscar: ")

        # Buscar paciente por documento
        paciente_encontrado = Paciente.buscar_paciente_por_documento(documento_paciente_buscar)
        ...
        if paciente_encontrado:
            ...
            imprimir_animacion(f"Fecha de Nacimiento: {paciente_encontrado.fecha_nacimiento}\n")
            ...
            imprimir_animacion(f"No se encontró un paciente con el documento {documento_paciente_buscar}.\n")
        ...
        documento_paciente_eliminar = input("Ingrese el documento del paciente que desea eliminar: ")
        paciente_eliminado=Paciente.eliminar_paciente_por_documento(documento_paciente_eliminar)
        if paciente_eliminado:
            imprimir_animacion("Paciente eliminado con exito")
            cantidad_altas+=1
            Cama.cantidad_camas+=1
        else:
            imprimir_animacion(f"No se encontro un paciente con el documento {documento_paciente_eliminar}")
        ...
        limpiar_consola()
        imprimir_animacion("-"*30)
        imprimir_animacion(f"Cantidad de admisiones: {cantidad_admisiones}\n")
        imprimir_animacion("-"*30)
        imprimir_animacion(f"Cantidad de altas: {cantidad_altas}\n")
        imprimir_animacion("-"*30)
        imprimir_animacion(f"Cantidad de camas disponibles: {Cama.cantidad_camas}\n")
        imprimir_animacion("-"*30)
        imprimir_animacion(f"Cantidad de enfermedades cronicas: {cantidad_enfermedades_cronicas}")
        imprimir_animacion("-"*30)
...
def ingresar_paciente():
    ...
    while True:
        ...
        resultados_laboratorio = input("- Resultados de laboratorio (Presenta enfermedad cronica? Escribir si/no): ")
        ...
        print("")
        while True:
            nombre_medicamento = input("- Nombre del medicamento (o presione Enter para terminar): ")
        ...
        nueva_historia_clinica = HistoriaClinica(presion_arterial, temperatura, saturacion_o2, frecuencia_respiratoria, notas_evaluacion, imagenes_diagnosticas, resultados_laboratorio)
        ...
        nuevo_paciente = Paciente(documento_paciente, nombre_paciente, sexo, fecha_nacimiento)              
